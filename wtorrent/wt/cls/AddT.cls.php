<?php
/*
 * This file is part of wTorrent. wTorrent is free software; you can
 * redistribute it and/or modify it under the terms of the GNU General Public
 * License as published by the Free Software Foundation; either version 3 of the
 * License, or (at your option) any later version. wTorrent is distributed in
...
class AddT extends rtorrent {
	protected static $MAGNET_REGEX = '/^magnet:\?xt=urn:.*/is';
	...
	private function addRemoteTorrent($url, $dir, $start_now, $private) {
		...
		$uploadfile = '';
		if (! preg_match ( self::$MAGNET_REGEX, $url )) {
			// Parsing url
			$purl = parse_url ( $url );
	...
	private function getCookie($url) {
		...
		$purl = parse_url ( $url );
		return implode ( ';', $this->_db->queryColumnAll ( 'SELECT value FROM cookie WHERE userid = ? AND hostname LIKE ?', $this->getIdUser (), "%{$purl['host']}%" ) );
	}
